...
class SemanticAnalyzer(Visitor):
    ...
    def visit_var_assign(self, node):
        """
        Enhanced var_assign with support for list and group validation.
        """
        children = node.children
        ident = children[0]
        line = ident.line
        ...
        # Update if this is a simple variable (not list/group element)
        if not has_accessor:
            self.current_scope.variables[name].value = expr_value
        
        print(f"Variable {name} {'element ' if has_accessor else ''}reassigned to expression value {expr_value[1]} and type {expr_value[0]}")
        return
